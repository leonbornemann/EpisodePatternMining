...
public abstract class EpisodePatternMiner<E extends EpisodePattern> {
	...
	public Map<E,Integer> mineEpisodePatterns(int s, int n){
		System.out.println("Starting to mine parallel episodes out of "+pred.size() + " windows with support "+s );
	...
	private Map<E,Integer> getBestPredictors(Map<E, Integer> frequent, int n) {
		...
		Map<E, Integer> trustScore = frequent.entrySet().stream().collect(Collectors.toMap(e -> e.getKey(), e -> e.getValue() - supportForInverse.get(e.getKey())));
