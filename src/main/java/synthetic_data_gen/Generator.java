...
public class Generator {
	...
	public Generator(double p, int N, Set<AnnotatedEventType> eventAlphabet,AnnotatedEventType toPredict , int n, int T, int g, double o, double G,int m, NoiseKind P, boolean r, boolean s, boolean i, boolean W, boolean h, boolean S,StandardDistribution d, StandardDistribution D) {
		super();
		this.p = p;
		this.N = N;
	...
	private void embedNextEvent(LocalDateTime timestamp, SerialEpisodePattern chosen) {
		...
		stream.add(new AnnotatedEvent(nextEvent,timestamp));
	...
	private AnnotatedEvent getNoiseEvent(LocalDateTime timestamp) {
		if(P==NoiseKind.UNIFORM){
			return new AnnotatedEvent(sampleUniformly(eventAlphabet),timestamp);
		} else{
	...
	private void generateSourceEpisodes() {
		...
		for(int i=1;i<=n;i++){
			...
			List<AnnotatedEventType> selected = new ArrayList<>();
			for(int j=1;j<=N;j++){
				boolean accepted = false;
				AnnotatedEventType E = null;
				while(!accepted){
	...
	private AnnotatedEventType sampleUniformly(Collection<AnnotatedEventType> types) {
		ArrayList<AnnotatedEventType> typesAsList = new ArrayList<>(types);
		Collections.sort(typesAsList);
		return typesAsList.get(random.nextInt(types.size()));
	}

	private Set<AnnotatedEventType> getUnionOfContainedTypes(ArrayList<? extends EpisodePattern> sourceEpisodes) {
		Set<AnnotatedEventType> allTypes = new HashSet<>();
		sourceEpisodes.forEach(e -> allTypes.addAll(e.getAllContainedTypes()));
		return allTypes;
	}

	public AnnotatedEventStream getGeneratedStream() {
		return new InMemoryAnnotatedEventStream(stream);
	}
