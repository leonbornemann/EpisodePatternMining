...
public class Evaluator{
	...
	public Evaluator(List<File> lowLevelStreamDirs) throws IOException{
		...
		for (LocalDate day : daysSorted) {
			...
			for(String companyID : predictionsByCompanyByDay.keySet().stream().sorted().collect(Collectors.toList())){
				...
				if(byDay.get(day)!=null && !targetMovement.isEmpty()){
					evalThisNowBetter(byDay.get(day),day,targetMovement,result);					
				} 
				System.out.println("--------------------------------------------------------------------");
			}
			System.out.println("--------------------------------------------------------------------");
	...
	private void evalThisNowBetter(List<Pair<LocalDateTime,Change>> predictions, LocalDate day, TreeMap<LocalDateTime, BigDecimal> timeSeries, EvaluationResult evalResult) {
		List<BigDecimal> longResult = new ArrayList<>();
		List<BigDecimal> shortResult = new ArrayList<>();
		ConfusionMatrix result = evalMetrics(predictions, timeSeries, longResult, shortResult);
	...
	private ConfusionMatrix evalMetrics(List<Pair<LocalDateTime, Change>> predictions,
